...
The archive is also published to Maven Central under the project’s group
ID. You can download it directly with Maven or Gradle, or retrieve it
manually:

``` bash
# With curl via the Central repository search API
curl -LO "https://repo1.maven.org/maven2/com/haldish/haldish/<version>/haldish-<version>.run"
```
...
| Function                              | Description                                      |
|---------------------------------------|--------------------------------------------------|
| `hal::log::info <message>`            | Print a cyan `[INFO]` line.                      |
| `hal::log::ok <message>`              | Print a green `[ OK ]` line.                     |
| `hal::log::warn <message>`            | Print a yellow `[WARN]` line.                    |
| `hal::log::error <message>`           | Print a red `[ERR ]` line.                       |
| `hal::log::die [exit-code] <message>` | Print an error and exit. Default exit code is 1. |
