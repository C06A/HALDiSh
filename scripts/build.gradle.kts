// ─── paths ────────────────────────────────────────────────────────────────────
def srcDir      = file("src/main/bash")
def testDir     = file("src/test/bats")
def batsInstall = file("${buildDir}/bats")
def batsExe     = file("${batsInstall}/bin/bats")
def distDir     = file("${buildDir}/dist")
def archiveName = "${rootProject.name}-${version}.run"
 ...
 */
tasks.register('installBats', Exec) {
    group       = 'setup'
...
tasks.register('test', Exec) {
    ...
    def resultsDir = file("${buildDir}/test-results")
...
tasks.register('assembleDist') {
    ...
    doLast {
        ...
        def payloadTar  = file("${buildDir}/payload.tar.gz")
        def archiveFile = file("${distDir}/${archiveName}")
        ...
        def header = file("src/packaging/_archive_header.sh").text
        def payload = payloadTar.bytes.encodeBase64().toString()

        archiveFile.text = header +
