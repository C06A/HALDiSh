...
| `halcurie.sh` | `HAL_LINK_PLUGIN` that expands a proper W3C CURIE-prefixed link href (via uppercase `_links.CURIE`, distinct from HAL's lowercase `curies`). Plugin contract: stdin = link JSON, args = `<root-file> <hal-path-to-link…>`, stdout = link JSON with `.href` expanded. A href "looks like a CURIE" when it is `<prefix>:<reference>` with an NCName prefix; the prefix is resolved against the first link in an `_links.CURIE` collection (uppercase, singular — single object or array) whose `name` equals the prefix, searched from the link's resource up the embedded stack to the root. A CURIE href is a plain URL (no `{rel}` template), distinct from HAL's templated `curies`; expansion replaces the `<prefix>:` of the original href. The search re-queries the root file via `hal.sh` — it swaps the trailing `links <rel>` for `links CURIE <prefix> href` (array entry picked by `name`), falling back to a single `CURIE` object whose `name` it asserts equals the prefix, and drops the last `embeddeds …` segment on each miss to step up. A scheme with no matching CURIE (e.g. `http:`), an unknown prefix, a prefix-less or absent href, or a missing root-file arg all pass the link through unchanged (exit 0). Exit 4 = no yq/jq; 5 = invalid stdin JSON. |
