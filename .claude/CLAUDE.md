...
| `haldoclink.sh` | Emit a documentation link `{"href":"<doc_url>","type":"text/html"}` for a relation. Resolves the relation name against `_links.curies` (lowercase, HAL spec's templated link prefix mechanism — not a proper CURIE). Searches from the deepest embedded resource up to the root for the prefix definition. `<file> <hal-path>` only. |
| `halcurie.sh` | `HAL_LINK_PLUGIN` that expands a proper W3C CURIE-prefixed link href (via uppercase `_links.CURIE`, distinct from HAL's lowercase `curies`). Plugin contract: stdin = link JSON, args = `<root-file> <hal-path-to-link…>`, stdout = link JSON with `.href` expanded. Every plugin also honors `-config` as its first arg — it prints a shell snippet that recreates the plugin's environment from the current values (no stdin read, no expansion) and exits 0; `nahal.sh` records this per plugin so a session replay can restore or diff the env (`halcurie.sh` needs no env, so it emits nothing). Only a **SafeCURIE** href is expanded — a CURIE in square brackets, `[<prefix>:<reference>]` with an NCName prefix (e.g. `[doc:orders]`); the brackets are the W3C SafeCURIE marker that disambiguates a CURIE from a same-shaped URI (`http://host/…`), so a bare unbracketed href is always left unchanged. The prefix is resolved against the first link in an `_links.CURIE` collection (uppercase, singular — single object or array) whose `name` equals the prefix, searched from the link's resource up the embedded stack to the root. A CURIE definition href is a plain URL (no `{rel}` template), distinct from HAL's templated `curies`; expansion replaces the whole `[<prefix>:<reference>]` with that URL followed by the reference (brackets dropped). The search re-queries the root file via `hal.sh` — it swaps the trailing `links <rel>` for `links CURIE <prefix> href` (array entry picked by `name`), falling back to a single `CURIE` object whose `name` it asserts equals the prefix, and drops the last `embeddeds …` segment on each miss to step up. A bare (unbracketed) href, a SafeCURIE with an unknown prefix, a prefix-less or absent href, or a missing root-file arg all pass the link through unchanged (exit 0). Exit 4 = no yq/jq; 5 = invalid stdin JSON. |
| `halprepend.sh` | Prepend a base string to a HAL link object's `href`. Two modes: `--link <obj>` or `<file> <hal-path>`. Format-preserving (JSON/YAML/XML). Only a **relative** href is rewritten — an absolute href (one with a protocol `scheme:` or a protocol-relative `//host` domain) is passed through unchanged. |
...
| `nahal.sh` | Interactive HAL API browser. Fetches with the method commands, classifies responses by Content-Type, and navigates links/embeddeds/properties/docs — and arrays of resources — via `hal.sh`. Follows links with any HTTP method (HEAD/custom verbs via an on-demand `./<METHOD>` link). A link that carries a `deprecation` URL also offers an "open deprecation docs" action that opens that page in the browser, running the URL through `HAL_LINK_PLUGIN` first (same filters as a followed href). Each step numbers its response base once via `hal_basename.sh -p <prefix>` into `$_s` and passes `-s "$_s"` to both `hallink.sh` and the method (no `rename.sh`); the link-following pipeline `hallink.sh -s "$_s" <src>.body <path> [-- binds] | <METHOD> -s "$_s" --link` is identical live and in the replay, and `hallink.sh -s` also writes the `<base>.source/.halpath/.bindings` sidecars. The bare-URL start is named with `-s` but writes no sidecar. Logs a re-runnable `session.sh` whose steps capture each base into a `_b[]` array as a multi-line `_b[N]=$( … )` opening with `_s=$(hal_basename.sh -p "$_prefix")` — then one stage per indented line, `\`-continued (a leading `\|` after a bare newline is a Bash syntax error), with the `HTTP_IN_HEADERS=…` header a command-prefix on the method stage (no grouping subshells). `HTTP_IN_HEADERS` is emitted only for headers the link cannot supply — `httpreq.sh --link` derives `Accept` from the link's `type`, so the sole `Accept` written is on the initial bare-URL `GET`, plus `Content-Type` for body requests. `session.sh` holds only the recorded per-session values and the request steps; the session-independent replay machinery lives in a companion `session_prelude.sh` (written alongside it and `source`d at the top of `session.sh`) — argument parsing, the HALDiSh-environment bootstrap, `_ensure_method`, and the plugin list/env restore-and-diff checks. The prefix is set once into `$_prefix` (its recorded default in `session.sh`, the env/`-p` overrides applied by the prelude); at replay it can be overridden by `session.sh -p <prefix>` (wins) or an exported `HAL_FILE_PREFIX` (set-but-empty honored), else the baked value — bad args print usage and exit 2. The prelude opens with a bootstrap that self-activates the HALDiSh environment (uses it if on `PATH`, else sources `env.sh` from `$HAL_LIB_DIR` or `~/.local/lib/haldish`, else prints install instructions and exits). Link plugin support via `HAL_LINK_PLUGIN` (colon-separated script list): every followed link is piped through plugins in order before use (e.g. `halcurie.sh` expands CURIE-prefixed hrefs). Records the `HAL_LINK_PLUGIN` list at session creation and emits a `_check_plugins` that diffs it against the replay-time list (OK = in both, INFO = new, WARN = missing). Also records each plugin's environment by capturing its `<plugin> -config` output (a shell snippet, `%q`-encoded into `_plugin_cfg_<i>` beside a `_plugin_cfg_names` array) and captures — before `_restore_plugins` runs — whether the caller set `HAL_LINK_PLUGIN` at replay entry (`_hal_plugins_set_at_entry`); the emitted `_check_plugin_env` then, when the list was **unset** at entry (the session owns the env), `eval`s each recorded snippet to restore it (INFO), and when the list was **set** (the caller drives the env) only diffs each plugin's current `-config` against the recorded one (OK = match, WARN = differs, never overwriting). |
...
| `cleanup.sh` | Deletes a file group by base name, keeping listed extensions: `cleanup.sh <base> [keep-ext…]` (or `-- [keep-ext…]` reads base from stdin). Removes every `<base>.*` in the CWD whose extension is not in the keep list (no keep list → all). Prints the base name. |
