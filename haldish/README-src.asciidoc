...
This project contains BASH shell scripts to do HTTP requests to the server
and to extract information from the response.

These scripts can be combined into
common scenarios like Login, SSO, etc. or to execute and validate manual
or semi-automated tests.
It also could create the ASCIIdoc document with tagged ranges to include into
API documentation.
...
- create hardlinks to the `http.sh` script with HTTP Methods as names
...
This allows to combine these scripts into the pipeline.
...
[source,bash]
----
include::src/tests/scripts/specSetup.sh[tag=sourcingValidator]
----

This would be an only line, referencing the script from this package with explicit path. All follow calls to other script can be done by a name only.
...
The example script included in the Readme file on the top level.
More examples are located in the `example` module.
